##############################################################################
################### Version 7 added I-V calculations ########################
################### Sep 19,2020 ########################
############################################################################
...
for T in T_range:
    ...
    for E in E_range:
        ...
        if angleDependent:
            ...
            for d1 in range(segCount): # in each segment conduct the angular integration to obtain A,B,C and D
                fullSol[c1,c2,d1,0] = integrate.trapz(resMat[d1,0,:],th_range)# for each variable conduct the angle integration
    ...
    if IVCalculate:
        c3=0# Voltage index
        for V in V_range:
            TunnelI[c3,c2] = nm.trapz(cond[:,c2]*(getFermi(E_range,V) - getFermi(E_range,0)),E_range)
            c3 = c3 + 1
...
results = {'fullSol':fullSol,'cond':cond,'E_range':E_range,'T_range':T_range,'V_range':V_range,'TunnelI':TunnelI}
