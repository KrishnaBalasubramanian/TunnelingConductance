############################################################################
############## in version 3 of HTS - Semi transport conductance.  #####################
############## Divinding the interface into segments - Works for 2 segments ##########
##############################################################################
import numpy as nm
import cmath as cm
import pdb
...
def getSigT(th,E):
    ...
    for n in range(N_value):# number of segments in the calculation
        ...
        r = r+ 1
        M_matrix[r,c2:c2+8] =nm.array([-lam_l_p*u_l_p,lam_l_n*v_l_n,-lam_l_n*v_l_p,lam_l_p*u_l_n,(lam_r_p +2j*z)*u_r_p, -(lam_r_n - 2j *z)*v_r_n,(lam_r_p + 2j*z)*v_r_p, -(lam_r_n - 2j*z)*u_r_n])         
        r = r+ 1
        M_matrix[r,c2:c2+8] =nm.array([-lam_l_p*v_l_p,lam_l_n*u_l_n,-lam_l_n*u_l_p,lam_l_p*v_l_n,(lam_r_p +2j*z)*v_r_p, -(lam_r_n - 2j *z)*u_r_n,(lam_r_p + 2j*z)*u_r_p, -(lam_r_n - 2j*z)*v_r_n])         
        r= r+1
        c2 = c2 + 1 
    ...
    A_M = float(X[c2+1]*nm.conj(X[c2+1]))
    B_M = float(X[c2+2]*nm.conj(X[c2+2]))
